...
export default function ClassStatisticsModal({ classItem, isOpen, onClose }: ClassStatisticsModalProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: ['class-statistics', classItem.id],
    queryFn: () => apiClient.getClassStatistics(classItem.id),
    enabled: isOpen,
  });
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {data?.data?.averageAttendance?.toFixed(2) ?? '0.00'}
              </div>
              <div>
